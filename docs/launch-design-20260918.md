...
利用者の提案により、KirokunLaunch.storyboardのルートViewに指定していたLaunchBackgroundの塗りつぶしを削除。画像・サイズ・制約・起動方式は変更せず、一度に変える要素を背景色だけに限定する。PNG自体は不透明な赤い背景を含むため、画像が正常表示されれば見た目の赤色は残る。これは背景色が原因かを確認する検証変更であり、原因確定や実機での表示改善を意味しない。ユーザーデータの削除や独自の起動遅延は行わない。
